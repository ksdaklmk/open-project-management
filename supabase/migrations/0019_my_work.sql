-- Personal cross-workspace task query. RLS remains active because this RPC is
-- SECURITY INVOKER; it never assembles full per-workspace caches in clients.

create index tasks_assignee_updated_cursor_idx
  on tasks (assignee_id, updated_at desc, id desc);
create index tasks_assignee_due_cursor_idx
  on tasks (assignee_id, end_date, id)
  ...
  with scoped as (
    ...
    where p_cursor_sort is null
      or case when due_order
        then (sort_value, id) > (p_cursor_sort, p_cursor_id)
        else (sort_value, id) < (p_cursor_sort, p_cursor_id)
      end
  )
  select * from cursor_page
  order by
    case when due_order then sort_value end asc nulls last,
    case when not due_order then sort_value end desc nulls last,
    case when due_order then id end asc,
    case when not due_order then id end desc
  limit least(greatest(coalesce(p_limit, 100), 1), 200) + 1;
end;
$$;

revoke execute on function query_my_work(text, timestamptz, uuid, integer) from public;
grant execute on function query_my_work(text, timestamptz, uuid, integer) to authenticated;
